...
public:
	void convertCVtoEigen(const cv::Mat& mat_tvec_, const cv::Mat& R, Eigen::Vector3f& translation, Eigen::Quaternionf& orientation)
	{
		// This assumes that cv::Mats are stored as doubles. Is there a way to check this?
		// Since it_'s templated...
		translation = Eigen::Vector3f(float(mat_tvec_.at<double>(0, 0)), float(mat_tvec_.at<double>(0, 1)), float(mat_tvec_.at<double>(0, 2)));
	...
	{	
		...
		pub_position_space_ = nh_.advertise<geometry_msgs::Pose2D>("/block_finder/pose", 1);
		pub_point_pattern_ = nh_.advertise<geometry_msgs::PointStamped>("/block_finder/pose_point_pattern", 1);
		pub_point_world_ = nh_.advertise<geometry_msgs::PointStamped>("/block_finder/pose_point_world", 1);
		pub_block_size_ = nh_.advertise<std_msgs::Int32>("/block_finder/block_size_max", 1);
	...
	geometry_msgs::Pose2D rsjImageProcessing(cv::Mat& mat_img_input_c, cv::Mat& mat_img_input_g, int method = 0)
	{
		...
		if (method == 1)
		{
			...
			// 二値化する。
			cv::threshold(mat_img_input_g, mat_img_bin, int_thre_bin_, 255, cv::THRESH_BINARY);
		...
		else if(method == 2)
		{
			...
			cv::threshold(mat_img_mask, mat_img_mask_bin, int_thre_bin_, 255, cv::THRESH_BINARY);
			...
			if(int_max_area_num != -1)
			{
				...
				pose2d_block_disp.x = pose2d_block_image.x = static_cast<int>(param[0] + param[2]/2);
				pose2d_block_disp.y = pose2d_block_image.y = static_cast<int>(param[1] + param[3]/2);
	...
	void imageCb(const sensor_msgs::ImageConstPtr& msg)
	{
		...
		geometry_msgs::PointStamped point_block_pattern;  // 空間のブロックの位置（pattern座標系）
		geometry_msgs::PointStamped point_block_world;  // 空間のブロックの位置（world座標系）
		...
		if(!is_param_)
		{
			...
			if(patternfound)//検出できた場合
			{
				...
				try
				{
					...
					cv::solvePnP(ideal_points_, observation_points_, mat_k_, mat_d_, mat_rvec_, mat_tvec_, false);
					//回転ベクトルを回転行列へ変換する。
		...
		{
			// フレームをSendする。
			tf_broadcaster_.sendTransform(tf::StampedTransform(target_transform, ros::Time::now() - ros::Duration(0.1), camera_frame, target_frame));	
		}

		//画像を処理する。
		pose2d_block_image = rsjImageProcessing(mat_img_color, mat_img_gray, int_method);
		...
		if(!mat_tvec_.empty())
		{
			...
			//point_block_pattern.header.stamp = ros::Time::now();//ros::Time();
			//point_block_world.header.stamp = ros::Time::now();//ros::Time();
			...
			int int_count_horizontal = int(floor(point3f_area_max_.x/EST_RESO));
			//int int_count_vertical =  int(floor(point3f_area_max_.y/EST_RESO));
			...
			if(int(vec_point3f_block.size()) > 0)
			{
				try
				{
					...
					cv::projectPoints(vec_point3f_block, mat_rvec_, mat_tvec_, mat_k_, mat_d_, vec_point2f_block);
				}
			...
			{
				float f_error = sqrtf(pow(pose2d_block_image.x - vec_point2f_block.at(i).x, 2)+pow(pose2d_block_image.y - vec_point2f_block.at(i).y, 2));
			...
			try
			{
				...
				//tf_listener_.transformPoint(fixed_frame, begin_, point_block_pattern, camera_frame, point_block_world);
				tf_listener_.transformPoint(fixed_frame, point_block_pattern, point_block_world);//移動先のframe名、移動元、移動先
			}
			catch(tf::TransformException& ex)
			{
